...
CREATE TABLE IF NOT EXISTS LambertEcommerce.Users (
    ...
    CONSTRAINT users_email_valid_check CHECK (LambertEcommerce.Users.email ~ '^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+[.][A-Za-z]+$'::TEXT),
    ...
    CONSTRAINT users_insertedat_min_value_check CHECK (LambertEcommerce.Users.inserted_at >= NOW()),
    CONSTRAINT users_insertedat_updatedat_value_check CHECK (LambertEcommerce.Users.inserted_at <= LambertEcommerce.Users.updated_at)
    );

CREATE
OR REPLACE TRIGGER users_updatedat_trigger BEFORE
UPDATE
    ON LambertEcommerce.Users FOR EACH ROW
    EXECUTE
...
CREATE TABLE LambertEcommerce.Selling (
                                          ...
                                          CONSTRAINT selling_insertedat_updatedat_value_check CHECK (LambertEcommerce.Selling.inserted_at <= LambertEcommerce.Selling.updated_at)
);

CREATE
OR REPLACE TRIGGER selling_updatedat_trigger BEFORE
UPDATE
    ON LambertEcommerce.Selling FOR EACH ROW
    EXECUTE
    FUNCTION updatedat_set_timestamp_function ();
...
CREATE TABLE LambertEcommerce.Carts (
                                        ...
                                        CONSTRAINT carts_insertedat_updatedat_value_check CHECK (LambertEcommerce.Carts.inserted_at <= LambertEcommerce.Carts.updated_at)
);

CREATE
OR REPLACE TRIGGER carts_updatedat_trigger BEFORE
UPDATE
...
CREATE TABLE LambertEcommerce.Orders (
                                         ...
                                         CONSTRAINT orders_insertedat_updatedat_value_check CHECK (LambertEcommerce.Orders.inserted_at <= LambertEcommerce.Orders.updated_at)
);

CREATE
OR REPLACE TRIGGER orders_updatedat_trigger BEFORE
UPDATE
